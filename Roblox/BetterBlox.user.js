...
var declutterHamburger = true; // Declutters the left hand menu.
...
var removedBar = true;
var changeAccent = true // Change the accent colour. (experimental)
...
var darkMode = true; // Dark theme! (experimental)
...
(function() {
    ...
    log("Loaded on a page of type " + pageType + " (" +location.href+ ")")
    if (pageType == "library") {
        ...
        try {
            ...
            if (comments) {
               for (var i = 0; i < comments.children.length; i++) {
                    var comment = comments.children[i]
                    var removed = false
                    blockedWordsInComments.forEach(function(w) {
                        if ((comment.children[1].children[1].innerText.toLowerCase().includes(w.toLowerCase())|| comment.children[1].children[0].innerText == w) && removed == false) {
                            removed = true
                            comment.remove()
                            removedComments = removedComments + 1
                        } else {
                            if (comment.classList.contains("bb-scanned") == false) {
                                comment.classList.add("bb-scanned")
            ...
            if (logErrors) { log("Error",e) }
        }
        var rt = ""
        if (removedText) {
            rt = `/${document.querySelector(".comments").children.length + removedComments}, ${Math.floor((removedComments / (document.querySelector(".comments").children.length + removedComments))*100)}% `
        }
        if (removedBar) {
            rt = rt + ` <progress max="${document.querySelector(".comments").children.length + removedComments}" value="${removedComments}"></progress>`
        }
        document.querySelector("#AjaxCommentsContainer > div:nth-child(1) > h3:nth-child(1)").innerHTML = `${document.querySelector(".comments").children.length} Comments (BB removed ${removedComments}${rt})`
        var loadMoreButton = document.querySelector(".btn-control-sm.rbx-comments-see-more");
        if (loadMoreButton && autoLoadMore && !document.querySelector(".comments > .loading-animated")) {
